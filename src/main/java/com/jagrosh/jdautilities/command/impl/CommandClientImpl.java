...
public class CommandClientImpl implements CommandClient, EventListener
{
                             ...
                             GuildSettingsManager manager)
    {
        ...
        this.helpConsumer = helpConsumer==null ? (event) -> {
                StringBuilder builder = new StringBuilder("**"+event.getSelfUser().getName()+"** コマンド:\n");
                ...
                if(owner!=null)
                {
                    builder.append("\n\nその他のコマンドについては **").append(owner.getName()).append("**#").append(owner.getDiscriminator()).append(" までどうぞ。");
                    if(serverInvite!=null)
                        builder.append("\n またはサーバーに参加: ").append(serverInvite);
                }
                if(event.isFromType(ChannelType.TEXT))
                    event.reactSuccess();
                event.replyInDm(builder.toString(), unused -> {}, t -> event.replyWarning("コマンドヘルプを送信できませんでした。: DMを受信しないように設定していませんか？"));
        } : helpConsumer;
