...
public abstract class Command
{
    ...
    private final static String BOT_PERM = "%s %s の権限がこの %s で必要です";
    private final static String USER_PERM = "%s あなたは %s の権限を所持していない為実行できません: `%s";
     ...
     *
     * @param  event
     *         このコマンドをトリガーした {@link CommandEvent}。
     */
    protected abstract void execute(CommandEvent event);

    /**
     * {@link Command}を呼び出す{@link CommandEvent}を使って、コマンドをチェックします。
     * チェックが失敗した場合は、終了し、おそらく失敗メッセージで応答します。
     *
     * @param  event
     *         コマンドをトリガしたコマンドイベント
     */
    public final void run(CommandEvent event)
    {
        ...
        {
            terminate(event, "このコマンドはこのチャンネルでは使用できません!");
            return;
        }
        ...
        if(requiredRole!=null)
            ...
            {
                terminate(event, event.getClient().getError()+" このコマンドを実行するためには `"+requiredRole+"` の役職を持っている必要があります!");
                return;
            }
        ...
        if(event.getChannelType()==ChannelType.TEXT)
        {
            ...
            for(Permission p: botPermissions)
            {
                if(p.isChannel())
                {
                    if(p.name().startsWith("VOICE"))
                    {
                        ...
                        if(vc==null)
                        {
                            terminate(event, event.getClient().getError()+" このコマンドを実行するためにはボイスチャンネルに接続している必要があります!");
                            return;
                        }
                        else if(!event.getSelfMember().hasPermission(vc, p))
                        {
                            terminate(event, String.format(BOT_PERM, event.getClient().getError(), p.name(), "ボイスチャンネル"));
                            return;
                        }
                    }
                    else
                    {
                        ...
                        {
                            terminate(event, String.format(BOT_PERM, event.getClient().getError(), p.name(), "テキストチャンネル"));
                            return;
                        }
                    }
                }
                else
                {
                    if(!event.getSelfMember().hasPermission(event.getTextChannel(), p))
                    {
                        terminate(event, String.format(BOT_PERM, event.getClient().getError(), p.name(), "サーバー"));
                        return;
                    }
                }
            }
            ...
            for(Permission p: userPermissions)
            {
                if(p.isChannel())
                {
                    ...
                    {
                        terminate(event, String.format(USER_PERM, event.getClient().getError(), p.name(), "テキストチャンネル"));
                        return;
                    }
                }
                else
                {
                    ...
                    {
                        terminate(event, String.format(USER_PERM, event.getClient().getError(), p.name(), "サーバー"));
                        return;
                    }
                }
            }
        ...
        {
            terminate(event, event.getClient().getError()+" このコマンドはDMでは使用できません");
            return;
        }
        ...
        if(cooldown>0)
        {
            ...
            if(remaining>0)
            {
                ...
                if(error!=null)
                {
                    event.reply("クールダウン中でし\n" +
                            "あと " + remaining + "秒 待ってくだち");
                    return;
                }
            }
